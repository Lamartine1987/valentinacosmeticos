        ...
        if (userInfoContainer) {
            ...
            if (spans.length >= 2) {
                ...
                if (storeId && storeId !== 'all') {
                    if (storeId === 'Loja 1' || storeId === 'matriz') storeLabel = 'Loja 1';
    ...
    loadSellerStoreFilters() {
        ...
        selects.forEach(selectId => {
            ...
            if (selectEl) {
                ...
                html += '<option value="Loja 1">🏢 Apenas Loja 1</option>';
            ...
            selects.forEach(selectId => {
                ...
                if (selectEl) {
                    ...
                    uniqueStores.forEach(store => {
                        const storeName = store === 'Loja 1' || store === 'matriz' ? 'Loja 1' : 'Loja 2';
                        html += `<option value="${store}">🏢 Somente Loja ${storeName} (Consolidado)</option>`;
                    });
                    
                    html += '<optgroup label="Desempenho por Vendedor">';
                    sellers.forEach(s => {
                        const storeName = s.storeId === 'Loja 1' || s.storeId === 'matriz' ? 'Loja 1' : 'Loja 2';
    ...
    setupForm() {
        ...
        document.getElementById('form-sale').addEventListener('submit', async (e) => {
            ...
            if (isApiActive && saleId) {
                ...
                const targetStore = newSale.overrideStoreId || (this.currentUserProfile ? this.currentUserProfile.storeId : 'Loja 1');
