...
export const apiModule = {
    async sendWhatsAppMessage(phone, message, imageUrl = '', targetStoreId = 'Loja 1') {
        ...
        if (settings.instances && Array.isArray(settings.instances) && settings.instances.length > 0) {
             ...
             if (!inst || !inst.active) {
                 inst = settings.instances.find(i => i.storeId === 'Loja 1' || i.storeId === 'matriz'); // fallback
             }
             if (!inst || !inst.active) {
                 inst = settings.instances.find(i => i.active); // generic fallback
             }
             if (inst && inst.active) {
    ...
    async saveClient(clientData) {
        try {
            ...
            if (this.user && this.currentUserProfile) {
                ...
                enrichedClient.storeId = clientData.overrideStoreId || this.currentUserProfile.storeId || 'Loja 1';
            }
    ...
    async saveProduct(productData) {
        try {
            ...
            if (this.user && this.currentUserProfile) {
                ...
                enrichedProduct.storeId = this.currentUserProfile.storeId || 'Loja 1';
            }
    ...
    async saveSale(saleData) {
        try {
            ...
            if (this.user && this.currentUserProfile) {
                ...
                enrichedSale.storeId = saleData.overrideStoreId || this.currentUserProfile.storeId || 'Loja 1';
            }
                ...
                if (phoneStr) {
                    ...
                    if (!snap.empty) {
                        ...
                        await db.collection("leads").doc(leadDoc.id).update({
                            ...
                            storeId: enrichedSale.storeId || 'Loja 1', // Garante a posse do lead na conversão
